...
{include file=#headerTsalPath# title='T-SAL農園&nbsp;SNS'}
		...
		<div class="frame_page">
			...
			<div class="frame_groupHome">
				...
				<h3 class="memberColor">{$userName}&nbsp;さんの日記</h3>

{foreach from=$html item=var name=option}
{if $smarty.foreach.option.first}
				<p class="list_count_m">{$html|@count}件中&nbsp;<span id="min"></span><span>～</span><span id="max"></span>件を表示</p>
...
{/if}
						<tr>
							...
							<td class="td_myDiaryListTitle">
								<a href="./friendDiary.php?friendId={$friendId}&diaryId={$var.diaryId}&year={$var.year}&month={$var.month}">
{if $var.count == 0}
									{$var.title}
{else}
									{$var.title}({$var.count})

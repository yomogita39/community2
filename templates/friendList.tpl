...
{include file=#headerTsalPath# title='T-SAL農園&nbsp;SNS'}
		...
		<div class="frame_page">
			...
			<div class="frame_friendList">
				...
				<h3 class="h3_list_l">友達リスト({$html|@count})</h3>
...
{/if}
						<td class="list_td_l">
							<a href="./friendTimeLine.php?friendId={$var.friendId}" class="position_count">
								<img src="{$var.friendImage}" alt="{$var.friendName}画像" class="image_m">
{if 1 <= $var.friendUnread && $var.friendUnread <= 9}
								<span class="unread_count1">{$var.friendUnread}</span>
{elseif 10 <= $var.friendUnread && $var.friendUnread <= 99}
								<span class="unread_count2">{$var.friendUnread}</span>
{elseif 100 <= $var.friendUnread && $var.friendUnread <= 999}
								<span class="unread_count3">{$var.friendUnread}</span>
{/if}
							</a>
							<p><a href="./friendTimeLine.php?friendId={$var.friendId}">{$var.friendName}({$var.friendCnt})</a></p>
						</td>
